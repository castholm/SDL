...
`x86_64-macos-none`|❌|❌|🉑
`aarch64-macos-none`|❌|❌|🉑
`wasm32-emscripten-none`|🉑|🉑|🉑
`wasm64-emscripten-none`|🉑|🉑|🉑
...
>
> In addition, note that Emscripten 4.0.4 or later will provide its own official port of SDL3 if you pass `--use-port=sdl3` to `emcc`. Depending on your needs, **you might not even need this package at all**.
>
...
zig build -Dtarget=wasm32-emscripten-none --sysroot "$sysroot_path"
```
...
To build with [pthreads support](https://emscripten.org/docs/porting/pthreads.html), set the `emscripten_pthreads` option to `true`.
